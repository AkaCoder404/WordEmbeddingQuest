...
from six import text_type, PY2, iteritems, string_types
...
class Embedding(object):
    ...
    # Added Functions
    @staticmethod
    def from_gensim_word2vec(model):
        """
        Support Python 3.8 gensim.models.Word2Vec conversion
        """
        # Prepare to collect word counts and vectors
        word_count = {}
        vectors = []
        
        # Gensim 4.x uses .key_to_index and .index_to_key to handle vocabulary
        # Iterate through the sorted vocabulary by count in descending order
        for word in sorted(model.wv.key_to_index, key=lambda w: -model.wv.get_vecattr(w, 'count')):
            standardized_word = standardize_string(word)
            if standardized_word:
                vectors.append(model.wv[word])  # Get vector for the word directly
                word_count[standardized_word] = model.wv.get_vecattr(word, 'count')
        
        # Convert vectors list to a numpy array
        vectors = np.asarray(vectors)
        
        # Assuming CountedVocabulary and Embedding are defined elsewhere in your code
        vocab = CountedVocabulary(word_count=word_count)
        return Embedding(vocabulary=vocab, vectors=vectors)

    @staticmethod
    def from_gensim_keyedvectors(model, pretrained=False):
        """
        Support Python 3.8 gensim.models.KeyedVectors conversion
        """
        # Prepare to collect word counts and vectors
        word_count = {}
        vectors = []
        ...
        # Gensim 4.x uses .key_to_index and .index_to_key to handle vocabulary
        # Iterate through the sorted vocabulary by count in descending order
        for word in sorted(model.key_to_index, key=lambda w: -model.get_vecattr(w, 'count')):
            
            # Don't standardize if pretrained
            standardized_word = word
            if not pretrained:
                standardized_word = standardize_string(word)
            
            # standardized_word = standardize_string(word)
            if standardized_word:
                vectors.append(model[word])  # Get vector for the word directly
                word_count[standardized_word] = model.get_vecattr(word, 'count')
